...
Base.map(f::F, players::Players) where {F} =
    Base.map(f, players.players)
...
Base.@propagate_inbounds Base.setindex(players::Players, player, i::Int) =
    Players(Base.setindex(players.players, player, i))
